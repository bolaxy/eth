...
type Database struct {
	...
	diskdb bolaxydb.Sinker // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb bolaxydb.Sinker) *Database {
...
func NewDatabaseWithCache(diskdb bolaxydb.Sinker, cache int) *Database {
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	batch.SetMaxPendingTxns(bolaxydb.IdealBatchSize)
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			if err := batch.Set(db.secureKey(hash[:]), preimage); err != nil {
	...
	for size > limit && oldest != (common.Hash{}) {
		...
		if err := batch.Set(oldest[:], node.rlp()); err != nil {
			db.lock.RUnlock()
			return err
		}
...
func (db *Database) commit(hash common.Hash, batch bolaxydb.Batch) error {
	...
	if err := batch.Set(hash[:], node.rlp()); err != nil {
		return err
	}
...
func (db *Database) verifyIntegrity() {
	...
	unreachable := make([]string, 0, len(db.dirties))
