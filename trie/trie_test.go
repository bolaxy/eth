...
// Used for testing
func newEmpty() *Trie {
	trie, _ := New(common.Hash{}, NewDatabase(bolaxydb.NewMemDatabase()))
	return trie
}
...
func TestMissingRoot(t *testing.T) {
	trie, err := New(common.HexToHash("0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33"), NewDatabase(bolaxydb.NewMemDatabase()))
	if trie != nil {
...
func testMissingNode(t *testing.T, memonly bool) {
	diskdb := bolaxydb.NewMemDatabase()
...
func TestCacheUnload(t *testing.T) {
	...
	diskdb := &countingDB{Sinker: trie.db.diskdb, gets: make(map[string]int)}
	triedb := NewDatabase(diskdb)
	trie, _ = New(root, triedb)
...
func runRandTest(rt randTest) bool {
	triedb := NewDatabase(bolaxydb.NewMemDatabase())
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		ldb := trie.db.diskdb.(*bolaxydb.BadgerDatabase)
		ldb.Close()
...
func tempDB() (string, *Database) {
	...
	diskdb, err := bolaxydb.NewBadgerDatabase(dir)
	if err != nil {
