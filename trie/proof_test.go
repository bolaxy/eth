...
// test all variations.
func makeProvers(trie *Trie) []func(key []byte) *bolaxydb.MemDatabase {
	var provers []func(key []byte) *bolaxydb.MemDatabase

	// Create a direct trie based Merkle prover
	provers = append(provers, func(key []byte) *bolaxydb.MemDatabase {
		proof := bolaxydb.NewMemDatabase()
...
func BenchmarkProve(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		proofs := bolaxydb.NewMemDatabase()
...
func BenchmarkVerifyProof(b *testing.B) {
	...
	root := trie.Hash()
	var keys []string
	var proofs []*bolaxydb.MemDatabase
	for k := range vals {
		keys = append(keys, k)
		proof := bolaxydb.NewMemDatabase()
