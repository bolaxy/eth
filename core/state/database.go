...
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb bolaxydb.Putter) error
}
...
func NewDatabase(db bolaxydb.Sinker) Database {
...
func NewDatabaseWithCache(db bolaxydb.Sinker, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb bolaxydb.Putter) error {
